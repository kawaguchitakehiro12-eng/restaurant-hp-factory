...
import { displayOrEmpty } from "@/lib/admin/demo-url-import";
...
export function DemoImportReviewPanel({ result }: DemoImportReviewPanelProps) {
  const menuSummary =
    ...
    <div className="admin-import-review">
      ...
      {stats.total > 0 ? (
        <div className="admin-photo-stats">
          ...
          <span className="admin-photo-stats-total">合計 {stats.total}枚取得</span>
        </div>
      ) : null}

      {result.fetchNotes.length > 0 ? (
        <ul className="admin-import-notes">
          {result.fetchNotes.map((note) => (
            <li key={note}>{note}</li>
          ))}
        </ul>
      ) : null}
