...
export function DemoCustomerBillingPage() {
  const { subscription, demo } = useDemoCustomer();
  const daysLeft = daysUntil(subscription.minimumTermEndDate);

  return (
    <>
      <PageHeader title="契約・請求情報" description="プラン・契約期間・請求状況の確認" />
      ...
      <div className="admin-card max-w-xl">
        ...
        <div className="admin-billing-row">
          ...
          <StatusBadge
            label={cancellationNoticeLabels[subscription.cancellationNoticeStatus]}
            variant={
      ...
      <div className="mt-4 max-w-xl">
        <BillingContractPanel
          ...
          contactEmail={demo.email || demo.contactPersonName}
        />
      </div>
    </>
  );
